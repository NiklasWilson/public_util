...
apt -y update && apt -y upgrade && apt -y install curl gpg nano lshw wget build-essential git clang curl libssl-dev llvm libudev-dev make protobuf-compiler htop screen ca-certificates curl gnupg npm ufw software-properties-common && apt -y autoremove
